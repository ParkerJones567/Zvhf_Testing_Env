...
inline void Softmax(const SoftmaxParams& params,
  ...
  for (int i = 0; i < outer_size; ++i) {
    ...
    float max = std::numeric_limits<_Float16>::lowest();
    ...
    _Float16 sum = 0.0f16;
    for (int c = 0; c < depth; ++c) {
      const _Float16 exp_c = std::exp((input_fp16[i * depth + c] - max) *
                                   static_cast<_Float16>(params.beta));
      output_fp16[i * depth + c] = exp_c;
      sum += exp_c;
    }

    // Compute result.
    for (int c = 0; c < depth; ++c) {
      output_fp16[i * depth + c] = output_fp16[i * depth + c] / sum;
    }
  }
}
