...
//This implementation DOESN'T convert to fp32.  ALL INTERMEDIATE OPERTIONS ARE DONE IN FP16
//Dequantize is now effectively a memcpy from input to output buffer.  TODO: with proper support, this could result in improved performance for fp16
template <>
inline void Dequantize<_Float16, float>(const tflite::DequantizationParams& op_params,
                       const RuntimeShape& input_shape,
                       const _Float16* input_data,
                       const RuntimeShape& output_shape, float* output_data) {
 // __asm__ volatile("jalr x0, 120(x0)");     //jump to 0x7c to signal success (Custom use interrupt, will not be called)
  const int flat_size = MatchingFlatSize(input_shape, output_shape);
  _Float16* output_fp16 = (_Float16*) output_data;
  
  //The copy to the output buffer operation seems to be unneeded.  Gives correct output when removed.  However, Spike still gives correct output when it is there.  Possible due to 'inline'
  //for (int i = 0; i < flat_size; i++) {
  //  output_fp16[i] = input_data[i];
  //} 
}
