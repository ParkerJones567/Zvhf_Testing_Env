...
//This implementation has _Float16 inputs.  TFLM treats them as float due to lack of _Float16 support for intermediate ops in TFLM + tflite
//Two major downsides of this approach:
// Tensor arenas are too large, could be reduced by ~1/2
// Cannot mix _Float16 and float ops
inline void FullyConnected(
 ...
 // __asm__ volatile("jalr x0, 120(x0)");  
    
  const _Float16* input_fp16 = (_Float16*)input_data;
  const _Float16* weights_fp16 = (_Float16*)weights_data;
  const _Float16* bias_fp16 = (_Float16*)bias_data;
  _Float16* output_fp16 = (_Float16*)output_data;
    
  const _Float16 output_activation_min = (_Float16)params.float_activation_min;
  const _Float16 output_activation_max = (_Float16)params.float_activation_max;
  ...
  for (int b = 0; b < batches; ++b) {
    for (int out_c = 0; out_c < output_depth; ++out_c) {
      _Float16 total = 0.0f16;
      for (int d = 0; d < accum_depth; ++d) {
        total += input_fp16[b * accum_depth + d] *
                 weights_fp16[out_c * accum_depth + d];
      }
      _Float16 bias_value = 0.0f16;
      if (bias_data) {
        bias_value = bias_fp16[out_c];
      }
      output_fp16[out_c + output_depth * b] = ActivationFunctionWithMinMax(
          total + bias_value, output_activation_min, output_activation_max); //Template Function, is generated for _Float16
    }
  }
}
